...
#define  libnvidia_ml "libnvidia-ml.so"
            ...
            nvmlh->nvmlDeviceGetAPIRestriction = (nvmlReturn_t (*)(nvmlDevice_t, nvmlRestrictedAPI_t,
                                                                   nvmlEnableState_t *))
                    wrap_dlsym(nvmlh->nvml_dll, "nvmlDeviceGetAPIRestriction");
            nvmlh->nvmlDeviceSetAPIRestriction = (nvmlReturn_t (*)(nvmlDevice_t, nvmlRestrictedAPI_t,
                                                                   nvmlEnableState_t))
                    wrap_dlsym(nvmlh->nvml_dll, "nvmlDeviceSetAPIRestriction");
            nvmlh->nvmlDeviceGetDefaultApplicationsClock = (nvmlReturn_t (*)(nvmlDevice_t, nvmlClockType_t,
                                                                             unsigned int *clock))
            ...
            nvmlh->nvmlDeviceSetApplicationsClocks = (nvmlReturn_t (*)(nvmlDevice_t, unsigned int mem,
                                                                       unsigned int gpu))
            ...
            nvmlh->nvmlDeviceGetSupportedGraphicsClocks = (nvmlReturn_t (*)(nvmlDevice_t, uint32_t mem, uint32_t *num,
                                                                            uint32_t *))
                    wrap_dlsym(nvmlh->nvml_dll, "nvmlDeviceGetSupportedGraphicsClocks");
            nvmlh->nvmlDeviceGetSupportedMemoryClocks = (nvmlReturn_t (*)(nvmlDevice_t, unsigned int *count,
            ...
            nvmlh->nvmlDeviceGetPciInfo = (nvmlReturn_t (*)(nvmlDevice_t, nvmlPciInfo_t *)) wrap_dlsym(nvmlh->nvml_dll,
            ...
            nvmlh->nvmlDeviceGetPowerManagementLimitConstraints = (nvmlReturn_t (*)(nvmlDevice_t, unsigned int *min,
                                                                                    unsigned int *max))
            ...
            nvmlh->nvmlDeviceGetUtilizationRates = (nvmlReturn_t (*)(nvmlDevice_t, nvmlUtilization_t *))
                    wrap_dlsym(nvmlh->nvml_dll, "nvmlDeviceGetUtilizationRates");
...
#endif
            // v346
            nvmlh->nvmlDeviceGetPcieThroughput = (nvmlReturn_t (*)(nvmlDevice_t, nvmlPcieUtilCounter_t,
                                                                   unsigned int *value))
                    wrap_dlsym(nvmlh->nvml_dll, "nvmlDeviceGetPcieThroughput");
            // v36x (API 8 / Pascal)
            nvmlh->nvmlDeviceGetClock = (nvmlReturn_t (*)(nvmlDevice_t, nvmlClockType_t clockType,
            ...
            if (nvmlh->nvmlInit == NULL ||
                ...
                nvmlh->nvmlDeviceGetName == NULL) {
                    printf("Failed to obtain required NVML function pointers");
...
/* apply config clocks to an used device */
        int nvml_set_clocks(nvml_handle *nvmlh, int dev_id) {
            ...
            if (nvmlh->app_clocks[n] != NVML_FEATURE_ENABLED) {
                printf("GPU #%d: NVML application clock feature is not allowed!", dev_id);
                return -EPERM;
            }
            ...
            if (rc != NVML_SUCCESS) {
                printf("GPU #%d: unable to query application clocks", dev_id);
                return -EINVAL;
            }

            printf("GPU #%d: default application clocks are %u/%u", dev_id, mem_clk, gpu_clk);
            ...
            nclocks = std::min(nclocks, static_cast<uint32_t>(32));
            if (nclocks)
            ...
            if (nclocks) {
                printf("GPU #%d: %u clocks found for mem %u", dev_id, nclocks, mem_clk);
...
//                applog(LOG_INFO, "GPU #%d: application clocks set to %u/%u", dev_id, mem_clk, gpu_clk);
                ...
                printf("GPU #%d: %u/%u - %s (NVML)", dev_id, mem_clk, gpu_clk, nvmlh->nvmlErrorString(rc));
                return -1;
            }
...
/* reset default app clocks and limits on exit */
        int nvml_reset_clocks(nvml_handle *nvmlh, int dev_id) {
            ...
            if (clock_prev[dev_id]) {
                ...
                if (rc != NVML_SUCCESS) {
                    printf("GPU #%d: unable to reset application clocks", dev_id);
                }
                clock_prev[dev_id] = 0;
                ret = 1;
            }
 ...
 */
        int nvml_set_pstate(nvml_handle *nvmlh, int dev_id) {
            ...
            if (nvmlh->app_clocks[n] != NVML_FEATURE_ENABLED) {
                printf("GPU #%d: NVML app. clock feature is not allowed!", dev_id);
                return -EPERM;
            }
            ...
            if ((uint32_t) wanted_pstate + 1 > nclocks) {
                printf("GPU #%d: only %u mem clocks available (p-states)", dev_id, nclocks);
            }
            for (uint8_t u = 0; u < nclocks; u++) {
            ...
            rc = nvmlh->nvmlDeviceSetApplicationsClocks(nvmlh->devs[n], mem_clk, gpu_clk);
            if (rc != NVML_SUCCESS) {
                printf("GPU #%d: pstate P%d (%u/%u) %s", dev_id, (int) wanted_pstate,
                       mem_clk, gpu_clk, nvmlh->nvmlErrorString(rc));
                return -1;
            }

//            if (!opt_quiet)
//                applog(LOG_INFO, "GPU #%d: app clocks set to P%d (%u/%u)", dev_id, (int) wanted_pstate, mem_clk,
//                       gpu_clk);
        ...
        int nvml_set_plimit(nvml_handle *nvmlh, int dev_id) {
            ...
            plimit = std::min(plimit, static_cast<uint32_t >(pmax));
            plimit = std::max(plimit, static_cast<uint32_t >(pmin));
            rc = nvmlh->nvmlDeviceSetPowerManagementLimit(nvmlh->devs[n], plimit);
            if (rc != NVML_SUCCESS) {
                printf("GPU #%d: plimit %s", dev_id, nvmlh->nvmlErrorString(rc));
                return -1;
            }
...
#define LSTDEV_PFX "        "
        ...
        void nvml_print_device_info(int dev_id) {
            ...
            if (hnvml->nvmlDeviceGetClock) {
                ...
                hnvml->nvmlDeviceGetClock(hnvml->devs[n], NVML_CLOCK_GRAPHICS, NVML_CLOCK_ID_APP_CLOCK_DEFAULT,
                                          &gpu_clk);
                rc = hnvml->nvmlDeviceGetClock(hnvml->devs[n], NVML_CLOCK_MEM, NVML_CLOCK_ID_APP_CLOCK_DEFAULT,
                                               &mem_clk);
                if (rc == NVML_SUCCESS) {
                    fprintf(stderr, LSTDEV_PFX "DEFAULT MEM %4u GPU %4u MHz\n", mem_clk, gpu_clk);
                }
                hnvml->nvmlDeviceGetClock(hnvml->devs[n], NVML_CLOCK_GRAPHICS, NVML_CLOCK_ID_APP_CLOCK_TARGET,
                                          &gpu_clk);
                rc = hnvml->nvmlDeviceGetClock(hnvml->devs[n], NVML_CLOCK_MEM, NVML_CLOCK_ID_APP_CLOCK_TARGET,
                                               &mem_clk);
                if (rc == NVML_SUCCESS) {
...
/* Not Supported on 750Ti 340.23 */
        int nvml_get_power_usage(nvml_handle *nvmlh, int cudaindex, unsigned int *milliwatts) {
            ...
            if (res != NVML_SUCCESS) {
                //if (opt_debug)
                //	printf("nvmlDeviceGetPowerUsage: %s", nvmlh->nvmlErrorString(res));
                return -1;
            }
...
/* Not Supported on 750Ti 340.23 */
        int nvml_get_pstate(nvml_handle *nvmlh, int cudaindex, int *pstate) {
            ...
            if (res != NVML_SUCCESS) {
                //if (opt_debug)
                //	printf("nvmlDeviceGetPerformanceState: %s", nvmlh->nvmlErrorString(res));
                return -1;
            }
        ...
        int nvml_get_serial(nvml_handle *nvmlh, int cudaindex, char *sn, int maxlen) {
            ...
            if (res != NVML_SUCCESS) {
//                if (opt_debug)
                    printf("nvmlDeviceGetUUID: %s", nvmlh->nvmlErrorString(res));
                return -1;
            }
            strncpy(sn, &uuid[4], std::min((int) strlen(uuid), maxlen));
            sn[maxlen - 1] = '\0';
            return 0;
        }
        ...
        int nvml_get_bios(nvml_handle *nvmlh, int cudaindex, char *desc, int maxlen) {
            ...
            if (res != NVML_SUCCESS) {
//                if (opt_debug)
                    printf("nvmlDeviceGetVbiosVersion: %s", nvmlh->nvmlErrorString(res));
                return -1;
            }
            return 0;
        }
...
//            }
        ...
        static int translate_vendor_id(uint16_t vid, char *vendorname) {
            ...
            if (/*opt_debug && */vid != 0x10DE)
                printf("nvml: Unknown vendor %04x\n", vid);
            return 0;
        }
